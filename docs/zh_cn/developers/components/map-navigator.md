...
- 多账号滑索：官方地图导入从 `/map/mark/list` 的 `roleId` 生成伪匿名账号标识，游戏运行时由场景初始化捕获 UID 并生成相同标识。规划器先按账号、再按地图筛选。旧版没有账号字段的记录不会被导入直接归属，而是在开启滑索寻路时由当前账号认领：当前账号名下一条记录都没有时，这些旧坐标整体归属给它并就地落盘，升级后无需重新导入；当前账号名下已有记录时不做认领，避免两套坐标混用后随换号静默失效。认领成功后会给出一条提示，详见 `ZiplineStore::claimLegacyRecords`。离线路线预览无法读取游戏 UID，可在「规划账号」中选择导入过的伪匿名账号，默认使用最近一次导入并在浏览器中记忆选择；该选择同时用于当前安装目录的滑索架图层，但不会写入复制参数或覆盖实机运行时账号。日志分析仍按对应运行日志中的账号标识过滤快照。Windows 端导入由 cpp-algo 的内嵌浏览器完成：任务开始时重新识别当前游戏 UID，并把 WebView2 登录状态分别保存到 `debug/record/WebView2/<account_id>`；识别不到 UID 时使用每次清空的隔离目录，直接显示网页登录界面，不会套用其他账号的状态。网页 `roleId` 与已识别的游戏账号不一致时整批拒绝落盘。Linux 端由 `agent/go-service/ziplineimport` 的本地受限 MITM 代理抄取同一接口，账号标识同样取自请求 URL 的 `roleId`，记录同样按 `(account_id, map_id)` 落盘，一次导入只接受唯一 `roleId`，否则整批拒绝。Linux 端每次导入都使用临时 Firefox 配置，登录态不会保留，因此「清除账号信息」选项在 Linux 控制器下不显示。
